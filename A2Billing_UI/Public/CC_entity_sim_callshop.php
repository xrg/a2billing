...
if ($called  && $id_cc_card){
		...
		if ( strlen($calling)>2 && is_numeric($calling)){
				...
				$resmax = $DBHandle -> query("SELECT username, tariff, activated FROM cc_card where id='$id_cc_card'");
				$num = $resmax -> numRows();
				...
				for($i=0;$i<$num;$i++)
					{
						$row [] =$resmax -> fetchRow();
						if (($FG_DEBUG>0 ) &&(!$row[$i][2]))
							echo "Card " .$row[$i][0] ." inactive! $i" ;
					}
				
				$A2B -> cardnumber = $row[0][0];
				if ($FG_DEBUG == 1) echo "cardnumber = ".$row[0][0] ." <br>";
				
				if ($A2B -> callingcard_ivr_authenticate_light ($error_msg,false )){
...
$instance_table_booth = new Table("cc_booth_v", "id, name, cur_card_id");
$FG_TABLE_CLAUSE = "cur_card_id IS NOT NULL";
$list_booths = $instance_table_booth -> Get_list ($DBHandle, $FG_TABLE_CLAUSE, "name", "ASC", null, null, null, null);
	  ...
	  <table width="<?php echo $FG_HTML_TABLE_WIDTH?>" border="0" align="center" cellpadding="0" cellspacing="0">
		...
		<tr>			
            <td height="31" bgcolor="#8888CC" style="padding-left: 5px; padding-right: 3px;">
					...
					<?php } ?>
					<br>
					<label> <?= _("Select the booth to charge"); ?> </label>
					<select name="id_cc_card" size="1" class="form_enter" style="border: 2px outset rgb(204, 51, 0);">
					<option value=''><?php echo gettext("-select booth-");?></option>
				
				<?php
			  	 foreach ($list_booths as $recordset){
				?> 
					<option class=input value='<?php echo $recordset[2]?>' ><?php echo $recordset[1]?></option>                        
				<?php 	 }
					?>
				</select>
