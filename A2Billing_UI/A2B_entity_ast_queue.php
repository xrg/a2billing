...
$HD_Form->model[] = new RefField(_("Autofill"), "autofill", $arr_yesnoN,_("Autofill will follow queue strategy but push multiple calls through
at same time until there are no more waiting callers or no more
available members."));
$HD_Form->model[] = new RefField(_("Auto Pause"), "autopause", $arr_yesnoN,_("Autopause will pause a queue member if they fail to answer a call"));
$HD_Form->model[] = new RefField(_("Set Intf. Var"), "setinterfacevar", $arr_yesnoN,_("Set the MEMBERINTERFACE variable with the interface name (eg. Agent/1234)"));
...
$HD_Form->model[] = new RefField(_("Avoid in use"), "ringinuse", $arr_yesnoN,_("If set to no, agents reported in use will not ring (SIP-wise)."));
$HD_Form->model[] = new IntFieldN(_("Member Delay"), "memberdelay",_("Seconds to delay before the member is connected to the caller."));
$HD_Form->model[] = new RefField(_("Timeout restart"), "timeoutrestart",$arr_yesnoN, _("Reset timeout for an agent to answer if a BUSY or CONGESTION is received."));
...
$HD_Form->model[] = new RefField(_("Announce holdtime"), "announce_holdtime", $arr_yno,_("Should we include estimated hold time in position announcements?"));
...
$HD_Form->model[] = new RefField(_("Hold time"), "reportholdtime", $arr_yesnoN,_("Report the caller's hold time to the member before they are
...
$HD_Form->model[] = new RefField(_("Format"), "monitor_format", $arr_monfmt,_("Record the calls. Please respect the law when using this."));
...
$HD_Form->model[] = new RefField(_("Join Empty"), "joinempty", $arr_joinempty,_("Allow/reject new callers if no members can answer their calls."));
$HD_Form->model[] = new RefField(_("Leave Empty"), "leavewhenempty", $arr_joinempty,_("Remove existing callers if no queue members."));

$HD_Form->model[] = new TabField(_("Manager Events"));

$arr_ewc = array (array(null,_('Default')), array ('yes', _("Yes")), array ('no',_('No')),
	array ('vars','Vars'));
$HD_Form->model[] = new RefField(_("When called"), "eventwhencalled", $arr_ewc,_("Generate manager events when called."));
$HD_Form->model[] = new RefField(_("Member status"), "eventmemberstatus", $arr_yesnoN,_("Report member status (generates MANY events)"));
