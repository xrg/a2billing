...
class phpmailer
{
	 ...
	 * A rotate system increment this number after each sending
     * @public
     * @type int
     */
    var $Curent_SMTP_Server      = -1;

	/**
     * Number of the try to send by SMTP
     * @public
     * @type int
     */
	var $nb_try=0;	
	...
	/**
     * Error received by SMTP Protocol
     * @public
     * @type int
     */
	var $Recup_code_error = null;
    ...
    /**
     *  Sets ROLL ROUND debugging on or off. Default value is false (off).
     *  @public
     *  @type bool
     */
    var $Debug_Roll    = false;
		  ...
		  while($this -> nb_try < count($hosts) && !$sendOKAY )
	        {
	          ...
	          if(!$this->smtp_send($header, $body)){
				  if (($this -> Recup_code_error == "550") && ($this -> nb_try < count($hosts)) ){
					  // Try again
					...
					//Envoie reussit -> SORTIR BOUCLE
					$sendOKAY=true;					//return true it's the same
			  }
    ...
    function smtp_send($header, $body) {
		...
		if ($this->Debug_Roll) print_r ($hosts);
		if (count($hosts)>1){
        ...
        while($this -> nb_try < count($hosts) && $connection == false)
        {
			...
			if ($this->Debug_Roll) echo "<br>  --> Host_USED: $host Port_USED: $port --->";


            if($smtp->Connect($host, $port, $this->Timeout)){
                $connection = true;
				if ($this->Debug_Roll) echo "<br>OK<br>";
			}else{
				//printf("<br><b> %s </b> :: host could not connect<br>", $hosts[$index]); //debug only
				if ($this->Debug_Roll) echo "<br>NO<br>";
        ...
        for($i = 0; $i < count($this->to); $i++)
        {
            if(!$smtp->Recipient(sprintf("<%s>", $this->to[$i][0]))){
				if ($smtp -> code_error != 550){ //Relaying denied
	                $bad_rcpt[] = $this->to[$i][0];
				}else{
					if ($this->Debug_Roll) echo "<br><br> Relaying denied !!!";
					if ($this->Debug_Roll) echo "<br>  --> Host_USED: $host Port_USED: $port --->";
					if ($this->Debug_Roll) echo "<br>Curent_SMTP_Server : ".$this -> Curent_SMTP_Server;
					if ($this->Debug_Roll) echo "<br> NB TRY : ".$this -> nb_try."<br>";
    ...
    */
    ...
    function received() {
        ...
        $str = sprintf("Received: from Mailing-List System [Version %s] " .
               ";\r\n\t %s\r\n",
               $this->Version,
               $this->rfc_date());

		/*$str = sprintf("Received: from Mailing-List System [Version %s] by %s " .
               "with HTTP (%s);\r\n\t %s\r\n",
               $this->Version,
