...
if(isset($pr_email) && isset($action))
{
    if($action == "email")
    {
        ...
        $show_message = true;
        $DBHandle  = DbConnect();
        $QUERY = "SELECT mailtype, fromemail, fromname, subject, messagetext, messagehtml FROM cc_templatemail WHERE mailtype='forgetagentpw' ";
        ...
        $QUERY = "SELECT name, login, passwd, email FROM cc_agent WHERE email='".$pr_email."' ";
	...
	if ($res)
        ...
        if($error == 0)
        {
            ...
            foreach ($list as $recordset)
            {
            	...
            	list($name, $username, $pass, $email) = $recordset;

            	if ($FG_DEBUG == 1) echo "<br># $name, $username, $email, $uipass #</br>";

				$messagetext = str_replace('$login', $username, $messagetext);
            	$messagetext = str_replace('$name', $name, $messagetext);
            	$messagetext = str_replace('$password', $pass, $messagetext);
